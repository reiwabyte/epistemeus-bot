export default async (clients, m, { isOwner, isGroup }) => {
    ...
    if (!target) return m.reply('Tag atau reply pesan anggota yang ingin di-demote.')

    try {
        await clients.groupParticipantsUpdate(m.chat, [target], 'demote')
        await clients.sendMessage(m.chat, {
            text: '@' + target.split('@')[0] + ' bukan admin lagi.',
